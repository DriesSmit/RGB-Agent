...
def _fire_analyzer(
    ...
    if actions_text:
        if agent.set_action_plan(actions_text):
...
def _log_action(prompts_log_path: Path, agent, action_num: int,
                level: int, attempt: int, arc_score: int, arc_state: GameState) -> None:
    if not prompts_log_path or not agent.trajectory.steps:
        return
    last_step = agent.trajectory.steps[-1]
    with open(prompts_log_path, 'a', encoding='utf-8') as f:
        f.write(f"\n{'='*80}\n")
        plan_info = f" | Plan Step {agent._plan_index}/{agent._plan_total}" if agent._plan_total > 0 else ""
...
def evaluate_single_game(
    ...
    try:
        ...
        if guid and not metrics.guid:
            metrics.guid = guid
            metrics.replay_url = f"{ROOT_URL}/replay/{game_id}/{guid}"
            log.info("[%s Run %d] Replay URL: %s", game_id, run_index, metrics.replay_url)
            if prompts_log_path:
                info_path = prompts_log_path.parent / "run_info.txt"
        ...
        # Main game loop
        while total_actions < max_actions_per_game:
            try:
                action_dict = loop.run_until_complete(agent.call_llm())
                ...
                for attempt in range(analyzer_retries):
                    ...
                    log.info("analyzer attempt %d/%d action=%d nudge=%s",
                             attempt + 1, analyzer_retries, total_actions, bool(nudge))
                    if _fire_analyzer(agent, analyzer, prompts_log_path, total_actions,
                                      arc_score, log_post_board, retry_nudge=nudge):
                        loaded = True
                        break
                    log.warning("analyzer attempt %d/%d failed — retrying", attempt + 1, analyzer_retries)
                if not loaded:
                    raise
                action_dict = loop.run_until_complete(agent.call_llm())
            ...
            _log_action(prompts_log_path, agent, total_actions, level_num, attempt_num, arc_score, arc_state)
            ...
            if arc_score > prev_score and arc_state not in (GameState.WIN, GameState.GAME_OVER):
                ...
                log.info("[%s Run %d] Level %d COMPLETED. Attempt %d actions: %d. Score: %d.",
                         game_id, run_index, level_num, attempt_num, attempt_metrics.actions, arc_score)
            ...
            if arc_state == GameState.GAME_OVER:
                ...
                log.warning("[%s Run %d] Game Over on Level %d, Attempt %d. Actions: %d.",
                            game_id, run_index, level_num, attempt_num, attempt_metrics.actions)
                attempt_num += 1
                attempt_metrics = AttemptMetrics(attempt_number=attempt_num)
                attempt_start = time.time()
            ...
            if arc_state == GameState.WIN:
                ...
                log.info("[%s Run %d] Game COMPLETED! Level %d actions: %d. Score: %d",
                         game_id, run_index, level_num, attempt_metrics.actions, arc_score)
                break

    except QueueExhausted as e:
        log.info("[%s Run %d] Episode ended (queue exhausted): %s", game_id, run_index, e)
        ...
        log.error("[%s Run %d] Exception: %s", game_id, run_index, e, exc_info=True)
        ...
        if metrics.guid and not metrics.replay_url:
            metrics.replay_url = f"{ROOT_URL}/replay/{game_id}/{metrics.guid}"
