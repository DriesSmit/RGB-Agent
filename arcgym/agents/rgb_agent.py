...
The Swarm creates one per game thread and calls agent.run().
"""
import asyncio
import logging
import os
...
class RGBAgent:
    """Drives the inner ClaudeCodeActionAgent through a game loop.

    Not a BaseAgent subclass — owns the env interaction loop,
    ...
    def run(self) -> GameMetrics:
        ...
        try:
            ...
            # Log initial board
            if self.prompts_log_path:
                grid = _render_board(self._agent)
                if grid:
    ...
    def _log_action(self, action_num: int, level: int, attempt: int,
        ...
        with open(self.prompts_log_path, 'a', encoding='utf-8') as f:
            f.write(f"\n{'='*80}\n")
            plan_info = f" | Plan Step {self._agent._plan_index}/{self._agent._plan_total}" if self._agent._plan_total > 0 else ""
